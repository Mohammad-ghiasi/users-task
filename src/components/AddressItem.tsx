    ...
    <>
      ...
      >
        <div className="w-full flex flex-col items-center md:items-start space-y-2">
          ...
          <p
            className="text-sm text-gray-600 sm:max-md:text-center truncate w-[85%]"
        ...
        {isOwn && (
          <div className="flex space-x-3 mt-2 sm:mt-0">
            <button onClick={() => setIsEditOpen(true)} className="text-[#499276] hover:text-[#3a7b5d] transition">
              <BiPencil className="text-xl md:text-2xl" />
            </button>
            <button
              onClick={() => deleteAddress(address._id)}
              className="text-red-500 hover:text-red-700 transition"
            >
              <BiTrash className="text-xl md:text-2xl" />
            </button>
          </div>
        )}
      </div>
